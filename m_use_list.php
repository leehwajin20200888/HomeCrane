...
//레코드 수량
if($search){
  $sql_ticket_use_info = "SELECT * FROM ticket_use WHERE USER_id = '".$_GET['search']."'";
  $sql_ticket_use_info_cnt = mysqli_query($con, $sql_ticket_use_info);
}else{
  $sql_ticket_use_info = "SELECT * FROM ticket_use";
  $sql_ticket_use_info_cnt = mysqli_query($con, $sql_ticket_use_info);
}
  $total = mysqli_num_rows($sql_ticket_use_info_cnt);
...
$startno = ($start - 1) * $listno; //첫번째로 읽어올 번호
  ...
  if ($search) {
    $ticket_use_info = "SELECT * FROM ticket_use WHERE USER_id = '".$_GET['search']."' ORDER BY TICKETUSE_day DESC LIMIT $startno, $listno";
  } else {
    $ticket_use_info = "SELECT * FROM ticket_use ORDER BY TICKETUSE_day DESC LIMIT $startno, $listno";
  }
  $payment_info_result = mysqli_query($con, $ticket_use_info);
    ...
    <table class="m_user_tb">
      ...
      ?>
              <tr>
                ...
                <td> <?php echo $row['TICKETUSE_day'] ?> </td>
                <td> <?php echo $row['TICKETUSE_use_cnt'] ?> </td>
                <td> <?php echo $row['TICKETUSE_remain_cnt'] ?> </td>
              </tr>
      <?php
            }
          }
