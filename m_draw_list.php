...
//레코드 수량
if($search){
  $sql_draw_list_info = "SELECT * FROM user_draw_list WHERE USER_id = '".$_GET['search']."'";
  $sql_draw_list_info_cnt = mysqli_query($con, $sql_draw_list_info);
}else{
  $sql_draw_list_info = "SELECT * FROM user_draw_list";
  $sql_draw_list_info_cnt = mysqli_query($con, $sql_draw_list_info);
}
  $total = mysqli_num_rows($sql_draw_list_info_cnt);
...
$startno = ($start - 1) * $listno; //첫번째로 읽어올 번호
  ...
  if ($search) {
    $user_draw_list_info = "SELECT * FROM user_draw_list WHERE USER_id = '".$_GET['search']."' ORDER BY DRAWLIST_day DESC LIMIT $startno, $listno";
  } else {
    $user_draw_list_info = "SELECT * FROM user_draw_list ORDER BY DRAWLIST_day DESC LIMIT $startno, $listno";
  }
  $user_draw_list_info_result = mysqli_query($con, $user_draw_list_info);
...
<body id="body-pd">
    ...
    <nav class="nav">
      <div class="sidebar_sort">
        ...
        <div class="nav__list">
          ...
          <a href="./m_draw_list.php" class="nav__link active">
  ...
  <div class="m_container">
    ...
    <table class="m_user_tb">
      ...
      <?php
        if($USER_id == 'soulmate'){
            while($row = mysqli_fetch_array($user_draw_list_info_result)){
      ?>
        <tr>
