...
//레코드 수량
if($search){
  $sql_ticket_buy_info = "SELECT * FROM ticket_buy WHERE USER_id = '".$_GET['search']."'";
  $sql_ticket_buy_info_cnt = mysqli_query($con, $sql_ticket_buy_info);
}else{
  $sql_ticket_buy_info = "SELECT * FROM ticket_buy";
  $sql_ticket_buy_info_cnt = mysqli_query($con, $sql_ticket_buy_info);
}
...
$startno = ($start - 1) * $listno; //첫번째로 읽어올 번호
  ...
  if ($search) {
    $ticket_buy_info = "SELECT * FROM ticket_buy WHERE USER_id = '".$_GET['search']."' ORDER BY TICKETBUY_day DESC LIMIT $startno, $listno";
  } else {
    $ticket_buy_info = "SELECT * FROM ticket_buy ORDER BY TICKETBUY_day DESC LIMIT $startno, $listno";
  }
  $ticket_buy_info_result = mysqli_query($con, $ticket_buy_info);
...
<body id="body-pd">
  ...
  <div class="m_container">
    ...
    <table class="m_user_tb">
      ...
      <?php
        if($USER_id == 'soulmate'){
            while($row = mysqli_fetch_array($ticket_buy_info_result)){
      ?>
              <tr>
                <td> <?php echo $row['USER_id'] ?> </td>
                <td> <?php echo $row['USER_name']?> </td>
                <td> <?php echo $row['TICKETBUY_num'] ?> </td>
                <td> <?php echo $row['TICKETBUY_day'] ?> </td>
                <td> <?php echo $row['TICKETBUY_cnt'] ?> </td>
              </tr>
      <?php
            }
          }
